...
#install.packages("C:/Users/darre/Downloads/DMwR_0.4.1.tar.gz", repos = NULL, type = "source")
...
new_wq <- SMOTE(is_safe ~., wq, perc.over = 1000, perc.under = 200)
table(new_wq$is_safe)
#new_wq$is_safe <- as.numeric(new_wq$is_safe)
barplot(table(new_wq$is_safe), xlab = 'is_safe', ylab = 'count')

# train test split
set.seed(13)
index <- createDataPartition(new_wq$is_safe, p = 0.8, list = FALSE)
#index = sample(nrow(new_wq), 0.80 * nrow(new_wq))
train <- new_wq[index,]
test <- new_wq[-index,]
...
# training model 2
model2 = glm(is_safe~. -flouride -lead -barium -nitrites, data = train, family = binomial)
summary(model2)
...
#p_opt <- ((18240 / (18240+10032)) + 0.5)/2
predictClass_2 = ifelse(predictTest_2 > 0.5,1,0)
...
#p_opt <- ((18240 / (18240+10032)) + 0.5)/2
predictClass_3 = ifelse(predictTest_3 > 0.5,1,0)
# table(test[,21],predictClass_2)
